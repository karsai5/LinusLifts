...
Template.body.events({
	'click #start-trip'(event) {
		...
		var pointLoop = setInterval(function() {
			...
			if (currentTrip()) {
				// get the actual current trip.
				trip = Trips.findOne(currentTrip()._id);
				geolocation = Geolocation.latLng();
				newdistance = trip.distance;

				// get new distance if more than one point
				if (trip.points.length > 0) {
					pointA = trip.points[trip.points.length-1];
					pointB = geolocation;
					newdistance = trip.distance
						+ distance(pointA.lat, pointA.lng, pointB.lat, pointB.lng, "K");
				}
